---
title: "Growth curves for strains with inducible sigma factors"
...
The reads were taken at 2 min intervals. To smooth the data we average over a sliding window, without overlaps, of 8 reads (~15min).
...
```
In all cases, it seems that the addition of IPTG at 4.5 hrs (gray vertical line), occurred prior to the transition into stationary phase.

There were a few instances of contamination (water used in no-IPTG samples), but very low, as evident by late rise of OD in those wells. Overall, 3 wells of 25 blanks setup. Removing blanks.
...
p  
```
Plot by strain, panel per colony.

```{r plot colony panels, message=FALSE}
col_numbers <-
  sum_colonies %>% 
...
# Plot
ggsave(here("fig/induction_growth_OD_colonies.png"),
       plot_grid(grob, labels = "a"),
       width = 8, height = 6)
